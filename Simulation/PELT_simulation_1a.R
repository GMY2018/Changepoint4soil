...
# load("PELT-sim-nlm-s1a.RData")
# load("Simulation-reID.RData")
# ID.rerun.s1
...
for (it in 1:N) {
  ...
  sm.sim[sID[1]:eID[1]] <- (runif(n=1, 0.1, 0.2) + delta.vec[1] - asym.vec[1])*(gamma.vec[1]^(0:sgl)) + 
    asym.vec[1] + rnorm(sgl+1, mean=0, sd=noise[1])
  for (i in 2:length(sID)) {
    sgl <- eID[i] - sID[i]
    sm.last <- sm.sim[eID[i-1]]
    sm.sim[sID[i]:eID[i]] <- (sm.last + delta.vec[i] - asym.vec[i])*(gamma.vec[i]^(0:sgl)) + 
      asym.vec[i] + rnorm(sgl+1, mean=0, sd=noise[1])
  }
  
  # Apply the algorithm
  time1 <- proc.time()
  # nlfb() function, with minsl update
  sm.pelt <- spike.PELT.msl2(data=sm.sim, xreg=0, ini.par=NULL, ini.asym=0.01,
                             pen=pen.sim, minsl=12, costfun=spike.exp.nlm1,
                             upper.par=c(0.5, 0.7, 1), thresh=0.0015, nprune=FALSE)
  # threshold = 3 * noise std
  pelt.s1r9 <- sm.pelt
  ...
  if (it %% 10 == 0) {
    save(spike.list, delta.list, gamma.list, asym.list, cpt.list, coef.list,
         mse.vec, bad.vec, timing.vec, file="PELT-sim-nlm-s1a.RData")
  }
}
